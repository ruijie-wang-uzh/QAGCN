...
def prep_kg() -> None:
    ...
    with open(args.in_path + "/PQL{}-KB.txt".format(hop), "r") as f:
        line = f.readline()
        while line:
    ...
    write_obj(obj=kg, obj_name="{}-hop knowledge graph".format(hop), file_path=os.path.join(args.out_path, "{}H-kg.pickle".format(hop)))
...
def prep_geo_graph() -> None:
    ...
    write_obj(obj=kg_graph, obj_name="{}-hop geometric graph".format(hop), file_path=os.path.join(args.out_path, "{}H-kg-geo_x.pickle".format(hop)))


def prep_subgraph_cans() -> None:
    print("\n* preparing {}-hop subgraphs and candidate targets ...".format(hop))
    kg = read_obj(obj_name="{}-hop knowledge graph".format(hop), file_path=os.path.join(args.out_path, "{}H-kg.pickle".format(hop)))
    kg_graph = read_obj(obj_name="{}-hop geometric graph".format(hop), file_path=os.path.join(args.out_path, "{}H-kg-geo_x.pickle".format(hop)))
    ...
    write_obj(obj=[ent2subgsz, ent2subg, ent2cans], obj_name="{}-hop entity subgraphs and candidate answers".format(hop), file_path=os.path.join(args.out_path, "{}H-kg-subg-cans.pickle".format(hop)))
...
def prep_ent_rel_embeds() -> None:
    ...
    with torch.no_grad():
        ...
        for rel_idx in range(num_rels):
            ori_label = id2rel[rel_idx].split('__')[-1].split("_")
            fil_label = []
    ...
    write_obj(obj=ent_embeds, obj_name="{}-hop entity embeddings".format(hop), file_path=os.path.join(args.out_path, "{}H-ent_label_embeds.pickle".format(hop)))
    write_obj(obj=rel_embeds, obj_name="{}-hop relation embeddings".format(hop), file_path=os.path.join(args.out_path, "{}H-rel_label_embeds.pickle".format(hop)))
