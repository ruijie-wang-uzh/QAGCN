...
class AttenModel(nn.Module):
    def __init__(self, device: torch.device, norm: int, ent_init_embeds: FloatTensor, rel_init_embeds: FloatTensor,
        ...
        self.ent_init_embed = torch.zeros(1, in_dims[0]).cuda()  # size: (num_ents, bert_output_size)
    ...
    def forward(self, que_embeds: FloatTensor, x: LongTensor, loc_edge_index: LongTensor, edge_attr: LongTensor, edge_type: LongTensor, top) -> FloatTensor:
        ...
        x = self.ent_init_embed.repeat(x.size(0), 1)  # size: (num_ents_in_subg, bert_output_size)
        x[top] = torch.ones(x.size(1)).cuda()

        r = self.rel_init_embeds  # size: (num_rels, bert_output_size)
        for l_id in range(self.num_layers):
            x, r = self.gcn_layers[l_id](x=x, r=r, que_context=que_context[l_id], edge_index=loc_edge_index.to(self.device), edge_attr=edge_attr.to(self.device), edge_type=edge_type.to(self.device))
...
class GCNLayer(nn.Module):
    ...
    def forward(self, x: FloatTensor, r: FloatTensor, que_context: FloatTensor, edge_index: LongTensor, edge_attr: LongTensor, edge_type: LongTensor) -> [FloatTensor, FloatTensor]:
        ...
        coeffs_sum = scatter(src=atten_coeffs, index=target_ents, dim=0, reduce="sum", dim_size=x.size(0))  # size: (num_ents_in_subg,)
        ...
        x = scatter(src=messages, index=target_ents, dim=0, reduce="sum", dim_size=x.size(0))  # size: (num_ents_in_subg, out_dim)
        x = self.e_bn(x)
        x = torch.tanh(x)
        x = self.dp(x)
...
class PathAlign3(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, norm: int, device: torch.device):
        super(PathAlign3, self).__init__()
        self.norm = norm
        self.device = device
    ...
    def forward(self, que_embed: FloatTensor, path_embeds: FloatTensor) -> FloatTensor:
        que_embed = que_embed.unsqueeze(1).to(self.device)  # size: (que_length, 1, bert_out_size)
        _, (_, que_embed) = self.que_lstm(que_embed)  # size: (1, 1, out_dim)

        _, (_, poss_path_embeds) = self.path_lstm(path_embeds)  # size: (1, num_poss_paths, out_dim)
        all_dis = torch.norm(que_embed.squeeze(0) - poss_path_embeds.squeeze(0), dim=1, p=self.norm)  # size: (num_poss_paths,)
        return torch.sigmoid(all_dis)

    def infer(self, que_embed: FloatTensor, path_embeds: FloatTensor) -> float:
        que_embed = que_embed.unsqueeze(1).to(self.device)  # size: (que_length, 1, bert_out_size)
        _, (_, que_embed) = self.que_lstm(que_embed)  # size: (1, 1, out_dim)

        _, (_, path_embed) = self.path_lstm(path_embeds)  # size: (1, 1, out_dim)
        dis = torch.norm(que_embed.view(-1) - path_embed.view(-1), dim=0, p=self.norm)  # size: (1,)
        return torch.sigmoid(dis)
