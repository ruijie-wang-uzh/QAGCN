...
class Main:
    def __init__(self):
        ...
        print("* loading data...")
        self.ent_embeds = read_obj(obj_name="entity embeddings", file_path=args.out_path + args.qa_type + "/{}H-ent_label_embeds.pickle".format(hop))
        self.rel_embeds = read_obj(obj_name="relation embeddings", file_path=args.out_path + args.qa_type + "/{}H-rel_label_embeds.pickle".format(hop))

        self.train_qid2batch = read_obj(obj_name="train qa batch", file_path=args.out_path + args.qa_type + "/{}H-train_qid2batch.pickle".format(hop))
        self.valid_qid2batch = read_obj(obj_name="valid qa batch", file_path=args.out_path + args.qa_type + "/{}H-valid_qid2batch.pickle".format(hop))
        self.test_qid2batch = read_obj(obj_name="test qa batch", file_path=args.out_path + args.qa_type + "/{}H-test_qid2batch.pickle".format(hop))

        self.train_qid2embeds = read_obj(obj_name="train question embeddings", file_path=args.out_path + args.qa_type + "/{}H-train_qid2embeds.pickle".format(hop))
        self.valid_qid2embeds = read_obj(obj_name="valid question embeddings", file_path=args.out_path + args.qa_type + "/{}H-valid_qid2embeds.pickle".format(hop))
        self.test_qid2embeds = read_obj(obj_name="test question embeddings", file_path=args.out_path + args.qa_type + "/{}H-test_qid2embeds.pickle".format(hop))

        self.valid_qid2tops = read_obj(obj_name="preprocessed valid questions", file_path=args.out_path + args.qa_type + "/{}H-valid_ques.pickle".format(hop))[1]
        self.test_qid2tops = read_obj(obj_name="preprocessed test questions", file_path=args.out_path + args.qa_type + "/{}H-test_ques.pickle".format(hop))[1]

        self.nx_kg = read_obj(obj_name="the networkx graph", file_path=args.out_path + args.qa_type + "/{}H-nx_kg.pickle".format(hop))
        self.id_paths, _, self.poss_path_embeds = read_obj(obj_name="ids, strings, and embeddings of possible shortest paths", file_path=args.out_path + args.qa_type + "/{}H-poss_paths.pickle".format(hop))
        self.id_path2idx = {}
    ...
    def train(self) -> None:
        ...
        if args.continue_train:
            ...
            atten_model.load_state_dict(torch.load(args.out_path + "models/{}-atten_model({}).pt".format(args.qa_type, args.timestamp), map_location=torch.device(args.device)))
...
if __name__ == "__main__":
    ...
    args.add_argument("--rerank_top", type=int, default=10)
