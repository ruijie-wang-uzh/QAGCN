...
class PathTrain:
    def __init__(self):
        ...
        self.model_path = args.out_path + "models/{}-path_align_model({}).pt".format(args.qa_type, datetime.now().strftime("%Y.%m.%d.%H.%M"))

        _, _, self.poss_path_embeds = read_obj(obj_name="ids, strings, and embeddings of possible shortest paths", file_path=args.out_path + args.qa_type + "/{}H-poss_paths.pickle".format(hop))
        self.qid2embeds = read_obj(obj_name="train question embeddings", file_path=args.out_path + args.qa_type + "/{}H-train_qid2embeds.pickle".format(hop))
        ...
        self.qid2path_dis = read_obj(obj_name="target train shortest path distances", file_path=args.out_path + args.qa_type + "/{}H-train_qid2path_dis.pickle".format(hop))
        self.valid_qid2path_dis = read_obj(obj_name="target valid shortest path distances", file_path=args.out_path + args.qa_type + "/{}H-valid_qid2path_dis.pickle".format(hop))
        self.test_qid2path_dis = read_obj(obj_name="target test shortest path distances", file_path=args.out_path + args.qa_type + "/{}H-test_qid2path_dis.pickle".format(hop))

    def train(self) -> None:
        print("### Training")
        path_align_model = PathAlign(poss_path_embeds=self.poss_path_embeds, in_dim=args.in_dim, out_dim=args.out_dim, norm=args.norm, device=args.device)
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_id, batch_data in enumerate(train_qa_loader):
                ...
                que_embed, path_dis = batch_data
                if torch.sum(path_dis == 0):

                    all_dis = path_align_model(que_embed=que_embed)
    ...
    def eval(self, mode: str, model: PathAlign) -> float:
        ...
        with torch.no_grad():
            ...
            eval_qa_loader = DataLoader(dataset=eval_qa_path_set, collate_fn=collate_fn, batch_size=1, shuffle=True)

            all_ranks = []
            all_ranks_inc_equ = []
            for batch_id, batch_data in enumerate(eval_qa_loader):
                que_embed, path_dis = batch_data
                all_dis = model(que_embed=que_embed)
                if torch.sum(path_dis == 0):
    ...
    def test(self) -> None:
        ...
        if args.num_epochs == 0:
            ...
            self.model_path = args.out_path + "models/{}-path_align_model({}).pt".format(args.qa_type, args.path_align_timestamp)
...
if __name__ == "__main__":
    ...
    args.add_argument("--out_dim", type=int, default=256)
