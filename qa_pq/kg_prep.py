...
from utils import read_obj, write_obj, stop_words, bert_encode
from argparse import ArgumentParser

def prep_kg() -> None:
    print("\n* preparing the knowledge graph for {} questions...".format(args.qa_type))
    ...
    with open(args.in_path + args.qa_type + "/{}H-kb.txt".format(hop), "r") as f:
        lines = f.readlines()
        for line in lines:
            line = line.strip()
            h, r, t = line.split()
    ...
    kg = {"num_ents": len(ent2id), "ent2id": ent2id, "num_rels": len(rel2id), "rel2id": rel2id, "num_trps": len(trp2id), "trp2id": trp2id}
    ...
    write_obj(obj=kg, obj_name="{}-hop knowledge graph".format(hop), file_path=args.out_path + args.qa_type + "/{}H-kg.pickle".format(hop))
...
def prep_geo_graph() -> None:
    ...
    write_obj(obj=kg_graph, obj_name="{}-hop geometric graph".format(hop), file_path=args.out_path + args.qa_type + "/{}H-kg-geo_x.pickle".format(hop))


def prep_subgraph_cans() -> None:
    print("\n* preparing {}-hop subgraphs and candidate targets ...".format(hop))
    kg = read_obj(obj_name="{}-hop knowledge graph".format(hop), file_path=args.out_path + args.qa_type + "/{}H-kg.pickle".format(hop))
    kg_graph = read_obj(obj_name="{}-hop geometric graph".format(hop), file_path=args.out_path + args.qa_type + "/{}H-kg-geo_x.pickle".format(hop))
    ...
    write_obj(obj=[ent2subgsz, ent2subg, ent2cans], obj_name="{}-hop entity subgraphs and candidate answers".format(hop), file_path=args.out_path + args.qa_type + "/{}H-kg-subg-cans.pickle".format(hop))
...
def prep_ent_rel_embeds() -> None:
    print("\n* preparing {}-hop entity and relation embeddings".format(hop))
    kg = read_obj(obj_name="{}-hop knowledge graph".format(hop), file_path=args.out_path + args.qa_type + "/{}H-kg.pickle".format(hop))
    num_ents = kg["num_ents"]
    ent2id = kg["ent2id"]
    ...
    with torch.no_grad():
        ent_labels = []
        for ent_idx in range(num_ents):
            ent_labels.append(id2ent[ent_idx].replace("_", " "))
    ...
    write_obj(obj=ent_embeds, obj_name="{}-hop entity embeddings".format(hop), file_path=args.out_path + args.qa_type + "/{}H-ent_label_embeds.pickle".format(hop))
    write_obj(obj=rel_embeds, obj_name="{}-hop relation embeddings".format(hop), file_path=args.out_path + args.qa_type + "/{}H-rel_label_embeds.pickle".format(hop))


if __name__ == "__main__":
    args = ArgumentParser()
    args.add_argument("--qa_type", type=str, default="2-hop", choices={'2-hop', '3-hop'})
    args.add_argument('--in_path', type=str, default='../data/pathquestion/input/', help='input data path')
    args.add_argument('--out_path', type=str, default='../data/pathquestion/output/', help='output data path')
