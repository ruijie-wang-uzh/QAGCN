...
from utils import QueSet, collate_fn, evaluate, read_obj, write_obj, stop_words

hops = [2, 3]
kg_path = '../data/pql-Xhop/output/XH-kg.pickle'
in_path = "../data/pql-Xhop/output/XH-Y_ques.pickle"
pre_data_path = "../data/pql-2hop/output/hop_prep_data.pickle"
model_path = "../data/pql-2hop/output/models/pql_hop_prep.pt"
dict_path = "../data/pql-2hop/output/dict.pickle"
discard = 15
lr = 0.0005
batch_size = 16
num_epochs = 50
...
if encode_ques:
    ...
    for hop in [2, 3]:
        ent2id = read_obj(file_path=kg_path.replace('X', str(hop)))['ent2id']
        id2ent = {v: k for k, v in ent2id.items()}
        train_ques = read_obj(in_path.replace('X', str(hop)).replace('Y', 'train'))
        valid_ques = read_obj(in_path.replace('X', str(hop)).replace('Y', 'valid'))
        test_ques = read_obj(in_path.replace('X', str(hop)).replace('Y', 'test'))
        for ques, q_set in zip([train_ques, valid_ques, test_ques], [train_set, valid_set, test_set]):
            que2id, qid2top = ques[0], ques[1]
            for que, qid in que2id.items():
                top = id2ent[qid2top[qid]]
                tmp_que = []
                for word in que.split(' '):
                    if word not in stop_words and word != top:
                        tmp_que.append(word)
                que = ' '.join(tmp_que)
                q_set.add(que=que, hop=hop, bias=2)
